...
const projects = [
  ...
  },
  {
    ...
    desc: 'Membangun akses dan jaringan pengadaan internasional untuk produk defence, teknologi rugged, dan 100+ brand aviasi global. POA, CoO, Manufacturer Statement.',
    tags: ['Defence', 'Aviation', 'Procurement', 'BD Internasional'],
    filter: 'rfa',
  },
  {
...
const procItems = [
  {
    ...
    items: ['Weibel Doppler Radar', 'Military Jammer — Omni & Falcon', 'Body-Worn Camera — Cysson', 'Pixhawk PX4 Speedometer', 'Sistem Baterai Militer'],
  },
  {
    num: '02',
    title: 'Teknologi & Rugged',
    sub: 'Enterprise & Field Ops',
    items: ['Microsoft HoloLens 1 & 2', 'Getac Rugged Devices', 'Toughbook Rugged', 'Maxhub Interactive Display', 'Oculus VR'],
  },
  {
    num: '03',
    title: 'Aviasi — 100+ Brand',
    sub: 'Global Aviation Supply',
    items: ['Garmin Aviation', 'Bose Aviation', 'Hartzell Propellers', 'Lycoming Engines', 'Dynon Avionics & lainnya'],
  },
];
      ...
      aside={(
        <div className="space-y-6">
          <div className="rounded-[1.75rem] border border-white/10 bg-ink/90 p-8">
            ...
            <blockquote className="mt-5 text-sm italic leading-7 text-white/70">
              &ldquo;Yang membedakan bukan seberapa banyak yang bisa dikerjakan, tapi{' '}
              <strong className="text-white not-italic">kemampuan membaca pola, merancang arahnya, lalu mengeksekusinya</strong>{' '}
    ...
    >
      <div className="space-y-10">
        ...
        <div className="flex flex-wrap gap-2">
          {filters.map((f) => (
            <button
              ...
              data-cursor-hover
              className={`rounded-full border px-5 py-2 font-mono text-[10px] uppercase tracking-[0.18em] transition ${
                active === f.key
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {visible.map((p) => (
            <article key={p.title} className="rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-[0_30px_80px_rgba(0,0,0,0.18)] transition hover:border-accent/30">
