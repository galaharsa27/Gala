...
function footer(lang = "id") {
  ...
  return `<footer class="site-footer">
    <div class="footer-grid">
      ...
      <div><b>${meta.social}</b><a href="https://www.instagram.com/shl.creative/" target="_blank" rel="noopener">Instagram</a><a href="mailto:Shlcreativeproduction@gmail.com">Email</a></div>
    </div>
...
<head>
  ...
  <meta property="og:image" content="https://shlcreative.com/assets/about-team.jpg">
  ...
  <meta name="twitter:image" content="https://shlcreative.com/assets/about-team.jpg">
  ...
  <script type="application/ld+json">${JSON.stringify({
    ...
    sameAs: ["https://www.instagram.com/shl.creative/"]
  })}</script>
</head>
...
function home(lang = "id") {
  ...
  return `<main>
    <section class="hero">
      <div class="hero-media"><iframe id="hero-video" src="https://www.youtube.com/embed/aAOPDXG4qBU?autoplay=1&mute=1&loop=1&playlist=aAOPDXG4qBU&controls=0&showinfo=0&modestbranding=1&rel=0&playsinline=1&disablekb=1&enablejsapi=1" title="SHL Creative Production hero video" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe></div>
...
function about(lang = "id") {
  ...
  return `<main>
    <section class="page-hero section"><div class="wrap"><div class="eyebrow">${isId ? "Tentang SHL" : "About SHL"}</div><h1>${isId ? "Ekosistem kreatif tempat ide menjadi makna." : "A creative ecosystem where ideas become meaning."}</h1><p>${isId ? "PT Samasta Hitakara Lekha menyatukan ide, manusia, teknologi, dan seni dalam satu semesta kolaborasi - mitra kreatif, bukan pabrik konten." : "PT Samasta Hitakara Lekha unites ideas, people, technology, and art in one universe of collaboration - a creative partner, not a content factory."}</p><figure class="about-team"><img src="/assets/about-team.jpg" alt="SHL Creative Production team"><figcaption><span>SHL Creative Production</span><span>Bandung, Indonesia</span></figcaption></figure></div></section>
    ...
    <section class="section" style="padding-top:30px"><div class="contact-grid wrap">
      ...
      <aside>
        ...
        <div class="direct">
          ...
          <a href="https://www.instagram.com/shl.creative/" target="_blank" rel="noopener"><strong>Instagram</strong><span>@shl.creative</span></a>
...
const js = `(() => {const header=document.getElementById("site-header");const btn=document.querySelector(".menu-button");const onScroll=()=>header&&header.classList.toggle("scrolled",window.scrollY>30);onScroll();window.addEventListener("scroll",onScroll,{passive:true});if(btn&&header){btn.addEventListener("click",()=>{const open=header.classList.toggle("open");btn.setAttribute("aria-expanded",String(open));});document.querySelectorAll(".site-nav a").forEach(a=>a.addEventListener("click",()=>{header.classList.remove("open");btn.setAttribute("aria-expanded","false");}));}const sound=document.getElementById("sound-toggle");const video=document.getElementById("hero-video");if(sound&&video){let on=false;const send=(func,args=[])=>{try{video.contentWindow.postMessage(JSON.stringify({event:"command",func,args}),"*")}catch(e){}};sound.addEventListener("click",()=>{on=!on;sound.classList.toggle("on",on);sound.setAttribute("aria-pressed",String(on));sound.querySelector("span").textContent=on?(sound.dataset.on||"Sound On"):(sound.dataset.off||"Sound Off");send(on?"unMute":"mute");if(on)send("setVolume",[100]);});}})();`;
